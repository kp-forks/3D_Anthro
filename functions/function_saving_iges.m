function function_saving_iges(obj,filename,varargin)
...
G.sendid         = '';                            % Product ID from Sender
G.receiveid      = '';                            % Product ID for Receiver
G.filename       = [file '.igs'];                 % File Name
G.systemid       = '';                            % System ID
G.preprocessor   = '';                            % Pre-processor Version
G.intbits        = 16;                            % Number of Bits for Integers
G.precision1     = 3;                             % Single Precision Magnitude
...
G.scale          = 1.0D0;                         % Model Space Scale
G.unitflag       = 2;                             % Unit Flag (1 = inches, 3 = look to units)
G.units          = 'MM';                          % Model Units 'MM' or 'IN' (Default)
G.lineweights    = 1;                             % Maximum number of line weights
G.linewidth      = 0.254D0;                       % Maximum line width
...
G.author         = 'W Lee';                       % Author
G.company        = 'Handong Global Univ.';        % Author's Organization
...
for i = 1:Num
    ...
    switch Entity(i)
        case 128
            P(1,i) = (i);             % Entity Type.  128 = Rational spline surface
...
if ~strcmp(filename(end-3:end), '.igs')
...
% if fclose(fid) == 0
%     disp([filename ' written successfully!']);
% end
