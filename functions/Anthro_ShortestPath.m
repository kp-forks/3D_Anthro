function curve = Anthro_ShortestPath(V, F, points, loop, simplified, smooth, interval)
    ...
    for i = 1:numPts-1
            curveRaw = [curveRaw; Anthro_ShortestPath_genPath(points(i, :), points(i+1, :), V, sparsematrix, 'Dijkstra')];
    end
        
    if loop == 0 % not closed curve        
    elseif loop == 1
        curveRaw = [curveRaw; Anthro_ShortestPath_genPath(points(end, :), points(1, :), V, sparsematrix, 'Dijkstra')];
    else
